...
void Dijkstra::Draw(CellSheetPainter* _pPainter)
{
	_pPainter->Paint(
...
CostAndPos Dijkstra::ToCostAndPos(const Vec2Int _pos)
{
	CostAndPos costAndPos{};
	costAndPos.cost += G(_pos);
	costAndPos.pos = _pos;

	return costAndPos;
}

float Dijkstra::G(const Vec2Int _pos)
{
	return roadCost_.GetCost(_pos);
}

float Dijkstra::GetDistance(const Vec2Int _a, const Vec2Int _b)
{
	// いったんマンハッタン距離
